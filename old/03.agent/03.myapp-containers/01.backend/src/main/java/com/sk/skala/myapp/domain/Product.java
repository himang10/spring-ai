...
public class Product {
    ...
    @ManyToOne(fetch = FetchType.LAZY)               // N:1 관계 - 여러 상품이 하나의 카테고리에 속함
    @JoinColumn(name = "category_id")                 // products 테이블에 category_id FK 컬럼 생성 (nullable)
    private Category category;                        // 상품 카테고리 (미지정 허용)
