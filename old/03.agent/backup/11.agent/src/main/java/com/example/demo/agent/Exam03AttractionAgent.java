...
public class Exam03AttractionAgent {
  ...
  private static final String SYSTEM_PROMPT = """
    ...
    ## 사용 가능한 도구
    1) searchAttractions: 관광지 후보 검색(요약) - 결과에 '🔑 상세조회ID(숫자): 123456' 형태의 숫자 ID가 포함됩니다.
    2) fetchAttractionInfo: 관광지 상세 정보 조회 - searchAttractions 결과의 '🔑 상세조회ID(숫자)' 항목에 있는 숫자만 입력해야 합니다.

    ## 규칙
    1) fetchAttractionInfo 호출 시 반드시 숫자 ID만 사용하세요. (예: 126508)
    2) URL 형태(http://... 또는 https://...)는 절대 사용하지 마세요.
    3) 관광지 후보는 최소 3개, 최대 6개를 제안하세요.
    4) entranceFee(입장료)는 가능한 한 도구 결과에서 찾아 채우세요.
    5) 도구 결과에서 입장료를 확인할 수 없다면 entranceFee는 0으로 두세요(후처리에서 보정됩니다).
    6) 자연/문화/체험 등 유형이 다양하도록 후보를 섞어 제안하세요.
  ...
  //-----------------------------------------------------------------------------------
  @Tool(description = "관광지 상세 정보를 조회합니다. searchAttractions 결과의 '🔑 상세조회ID(숫자)' 항목에 있는 숫자 ID만 입력하세요. URL(http://...)은 입력하면 안됩니다.")
  public String fetchAttractionInfo(@ToolParam(description = "searchAttractions 결과의 '🔑 상세조회ID(숫자)' 값 (예: 126508). 숫자만 입력하고 URL은 절대 사용하지 마세요.") String contentId) {
