...
public class DocumentEmbeddingService {
  ...
  public String addDocument(String request) {
    log.info("addDocument() called");
    // 단일 Document 생성
    Document document = new Document(
        request,
        Map.of("category", "append", "year", 2026)
    );
  ...
  public List<Document> search(String question) {
    ...
    List<Document> documents = vectorStore.similaritySearch(
        SearchRequest.builder()
            .query(question)
            .topK(1)
            .similarityThreshold(0.6) // 60% 이상 유사한 문서만 검색
            .build());
    return documents;
  }
  ...
  public List<Document> searchWithFilter(String question) {
    ...
    List<Document> documents = vectorStore.similaritySearch(
        SearchRequest.builder()
            ...
            .topK(1)
            .similarityThreshold(0.6) // 60% 이상 유사한 문서만 검색
            .filterExpression("category == 'spring' && year >= 2020")
            .build());
    return documents;
  }
