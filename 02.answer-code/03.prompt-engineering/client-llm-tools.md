...
## 목차
- [개요](#개요)
- [HTTP API Request 구조 (클라이언트 → LLM)](#http-api-request-구조-클라이언트--llm)
- [HTTP API Response 구조 (LLM → 클라이언트)](#http-api-response-구조-llm--클라이언트)
- [Tool Request/Response 개념 (Spring AI 관점)](#tool-requestresponse-개념-spring-ai-관점)
...
이 문서에서는 두 가지 관점의 요청/응답을 다룹니다:

1. **HTTP API Request/Response**: 클라이언트(Spring AI)와 LLM 간의 HTTP 통신
   - HTTP Request: 클라이언트 → LLM (사용자 메시지 + Tool 정의)
   - HTTP Response: LLM → 클라이언트 (답변 또는 Tool 호출 요청)

2. **Tool Request/Response**: Tool 실행 단계에서의 데이터 흐름 (Spring AI 관점)
   - **ToolRequest**: LLM이 Spring AI에게 Tool 실행을 요청 (`tool_calls` 필드)
   - **ToolResponse**: Spring AI가 Tool 실행 결과를 LLM에게 전달 (`role="tool"` 메시지)

## HTTP API Request 구조 (클라이언트 → LLM)

클라이언트(Spring AI)가 LLM에게 사용자 메시지와 사용 가능한 Tool 목록을 전달합니다.
...
## HTTP API Response 구조 (LLM → 클라이언트)

LLM이 클라이언트에게 반환하는 응답입니다. Tool 호출이 필요한 경우와 필요하지 않은 경우로 나뉩니다.
...
## Tool Request/Response 개념 (Spring AI 관점)

Spring AI 애플리케이션 관점에서 Tool 실행 과정의 Request/Response를 설명합니다.

### ToolRequest: LLM → Spring AI

LLM이 HTTP Response의 `tool_calls` 필드를 통해 "이 Tool을 실행해달라"고 Spring AI에 요청합니다.
...
| 필드 | 설명 |
|------|------|
| `id` | Tool 호출의 고유 ID (나중에 응답과 매칭하기 위해 사용) |
| `type` | 항상 `"function"` |
| `function.name` | 실행할 Tool의 이름 |
| `function.arguments` | Tool에 전달할 매개변수 (JSON 문자열) |

**Spring AI의 처리:**
1. `tool_calls` 배열에서 ToolRequest 추출
2. `function.name`에 해당하는 Java 메서드 찾기
3. `arguments`를 파싱하여 메서드 실행

### ToolResponse: Spring AI → LLM

Spring AI가 Tool을 실행한 후, 그 결과를 `role="tool"` 메시지로 LLM에게 전달합니다.
...
```json
{
  ...
  "content": "[실행 결과] 현재 시각: 오전 11시 10분"
}
```

| 필드 | 설명 |
|------|------|
| `role` | 항상 `"tool"` (메시지 타입 식별) |
| `tool_call_id` | ToolRequest의 `id`와 매칭 (어떤 요청에 대한 응답인지 식별) |
| `content` | Tool 실행 결과 (문자열 형태) |

**LLM의 처리:**
1. `tool_call_id`로 어떤 Tool 호출에 대한 결과인지 확인
2. `content`를 "Observation" 또는 "Context"로 활용
3. Tool 결과를 바탕으로 최종 사용자 답변 생성

### Tool 실행 흐름 (ToolRequest/ToolResponse 관점)

```
[1단계] 클라이언트 → LLM
        HTTP Request (사용자 질문 + Tool 정의)
        
        ↓
        
[2단계] LLM → 클라이언트  
        HTTP Response (tool_calls 포함)
        ┌─────────────────────────────────┐
        │ ToolRequest (LLM → Spring AI)   │  ← LLM이 Tool 실행 요청
        │  - id: "call_001"               │
...
[3단계] Spring AI에서 Tool 실행
        - getAllInfo() 메서드 호출
        - MCP Server 또는 로컬 서비스 호출
        - 결과 데이터 수집
        
        ↓
        
[4단계] 클라이언트 → LLM
        HTTP Request (이전 대화 + ToolResponse)
        ┌─────────────────────────────────────────┐
        │ ToolResponse (Spring AI → LLM)          │  ← Spring AI가 결과 전달
        │  - role: "tool"                         │
        │  - tool_call_id: "call_001"             │
        │  - content: "[실행 결과] ..."             │
        └─────────────────────────────────────────┘
...
[5단계] LLM → 클라이언트
        HTTP Response (최종 답변)
        - Tool 결과를 기반으로 생성된 자연어 답변
```

### 핵심 포인트

- **ToolRequest**: LLM이 "이 Tool을 실행해줘"라고 요청하는 단계
  - HTTP Response의 `tool_calls` 필드로 전달됨
  - Spring AI가 이를 해석하여 실제 Java 메서드나 MCP Server 호출

- **ToolResponse**: Spring AI가 "Tool 실행 결과는 이거야"라고 답변하는 단계
  - HTTP Request의 `messages` 배열에 `role="tool"` 메시지로 추가됨
  - LLM이 이를 관찰(Observation)로 사용하여 최종 답변 생성
...
[사용자 질문]
     ↓
[1. 클라이언트 → LLM] HTTP Request (messages + tools)
     ↓
[2. LLM → 클라이언트] HTTP Response (tool_calls 지시)
     │
     └─→ ToolRequest: LLM이 "getAllInfo 실행해줘"라고 요청
     ↓
[3. Spring AI가 Tool 실행] (MCP Server 또는 Java 메서드 호출)
     ↓
[4. 클라이언트 → LLM] HTTP Request (role=tool 메시지 추가)
     │
     └─→ ToolResponse: Spring AI가 "실행 결과는 이거야"라고 전달
     ↓
[5. LLM → 클라이언트] HTTP Response (최종 답변)
     ↓
[사용자에게 응답 표시]
```
