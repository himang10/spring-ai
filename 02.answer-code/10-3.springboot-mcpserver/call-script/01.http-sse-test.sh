...
# HTTP MCP Server - SSE (Stateful) Test Script
# SSE는 세션 상태를 유지하며 서버 푸시 이벤트를 지원
# 작업 완료 후 DELETE /mcp 필요

set -e

MCP_URL="http://localhost:8081/mcp"  # SSE 포트
QUESTION="모든 사용자 정보를 검색해줘"
SESSION_ID=""

echo "=========================================="
echo "HTTP MCP Server - SSE Test"
echo "=========================================="
...
echo "------------------------------------------"
echo ">>> REQUEST:"
echo "Receiving SSE stream (Server-Sent Events)..."
...
echo "=========================================="
echo "✓ SSE (Stateful) Test Completed"
echo "Note: SSE 프로토콜은 세션 상태를 유지하며"
echo "      실시간 서버 푸시 이벤트를 지원합니다."
