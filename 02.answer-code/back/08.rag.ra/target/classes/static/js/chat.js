...
// -----------------------------------------------------------------------------
// RAG: QuestionAnswer(/ai/rag/qa) / RewriteQueryTransformer(/ai/rag/rewrite)
//      / CompressionQueryTransformer(/ai/rag/compression) 중 선택하여 질의응답
// -----------------------------------------------------------------------------
const RAG_CONFIG = Object.freeze({
    ...
    rewrite: {
        ...
        // 구어체·감정 섞인 질문을 검색 친화적인 질문으로 재작성하는 예시.
        prompt: '국회의원이 하라는 일은 하지 않고, 자기 개인 이익만 챙기고 있고 이게 국회의원이 할 일이냐?',
        endpoint: '/ai/rag/rewrite',
        available: true
    },
    compression: {
        label: 'CompressionQueryTransformer',
        // 대화 이력은 세션 쿠키(conversationId)로 식별되어 서버에 누적된다.
        prompt: '국회의원의 역할과 책무는 무엇인가?',
        endpoint: '/ai/rag/compression',
        available: true
    }
});
...
ragForm.addEventListener('submit', async (event) => {
    ...
    // RetrievalAugmentation은 아직 백엔드 연동 전이므로 안내 문구만 표시한다.
    if (!config.available) {
        ragNoticeOutput.hidden = false;
        return;
    }
