...
/**
 * PDF 문서를 읽어(Extract) 청크로 분할하고(Transform) VectorDB에 저장(Load)하는
 * RAG ETL 학습용 서비스.
 *
 * 사용 예) pdfEtlService.extract("파일명.pdf").transform().load();
 */
@Service
@Slf4j
public class PdfEtlService {
	...
	private static final int CHUNK_SIZE = 800;               // 목표 토큰 수 (조문 1~2개 수준으로 축소)
	private static final int MIN_CHUNK_SIZE_CHARS = 350;     // 최소 문자 수 (짧은 조문도 별도 청크로 유지)
	...
	public PdfEtlService(ResourceLoader resourceLoader, VectorStore vectorStore) {
	...
	public PdfEtlService extract(String fileName) {
		...
		log.info("[PDF ETL] Extract 완료 - fileName: {}, document 수: {}", fileName, documents.size());
		return this;
	}

	/** Transform: TokenTextSplitter로 문서를 임베딩 모델에 맞는 크기의 청크로 분할한다. */
	public PdfEtlService transform() {
		...
		log.info("[PDF ETL] Transform 완료 - 청크 수: {}", documents.size());
		return this;
	}
	...
	public List<Document> load() {
		...
		log.info("[PDF ETL] Load 완료 - VectorDB 저장 건수: {}", documents.size());
		return documents;
	}
}
